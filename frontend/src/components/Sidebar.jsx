  ...
  return (
    <aside className="hidden lg:flex w-64 flex-col border-r border-slate-200/80 dark:border-slate-800/80 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl text-slate-900 dark:text-slate-100 min-h-screen sticky top-0 h-screen">
      ...
      <div className="flex items-center gap-3 border-b border-slate-200/80 dark:border-slate-800/80 p-5">
        ...
        <div>
          <p className="font-extrabold tracking-tight text-slate-900 dark:text-white font-display text-lg">GeoTrack</p>
      ...
      <div className="border-t border-slate-200/80 dark:border-slate-800/80 p-4 space-y-3">
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-slate-50 dark:bg-slate-950 p-3 border border-slate-200 dark:border-slate-800 shadow-sm">
          <img
            src={getImageUrl(user?.photo) || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=300&auto=format&fit=crop&q=80"}
            alt={user?.full_name}
            className="h-10 w-10 rounded-xl object-cover border-2 border-orange-500 shrink-0"
            onError={(e) => {
          ...
          <div className="truncate">
            <p className="text-xs font-extrabold text-slate-900 dark:text-white truncate font-display">{user?.full_name || "Employee"}</p>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono font-bold truncate">ID: {user?.employee_id}</p>
          </div>
        </div>
